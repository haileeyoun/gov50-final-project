...
library(readxl)
setwd("/Users/haileeyoun/Desktop/gov50-final-project")
EV <- read_excel("Electric_Vehicle_Population_Data.xlsx")
library(writexl)
write_xlsx(EV, "data_modified.xlsx")
```


```{r}
filtered_data <- EV |>
  group_by(City, `Model Year`) |>
  summarise(Count = n()) |>
  arrange(City, desc(`Model Year`)) |>
  group_by(City) |>
  top_n(1)  


ev_model_years_plot <- ggplot(data = filtered_data, aes(x = reorder(City, -`Model Year`), y = `Model Year`, fill = City)) +
  geom_bar(stat = "identity") +
  labs(
    x = "City",
    y = "Most Recent Model Year",
    title = "Most Recent Model Year of Electric Vehicles in Different Cities"
  ) +
  theme_minimal() 
...
scatter_plot <- ggplot(data = EV, aes(x = 'Electric Range', y = 'Model Year')) +
  geom_point() + 
  labs(
    x = "Electric Range",
    y = "Model Year",
    title = "Scatter Electric Range vs. Model Year"
  ) 
scatter_plot
```
